...
% http://ncsu.edu/foodscience/faculty/breidt_fred.htm
% 
% Function Input Parameters
% XYdata = an N x 2 vector with the first column of data being X values 
%   and the second column Y values 
% 
...
% the Y intercept (Initial CFU/mL at time zero), and Bhat(2) is the slope.
Bhat = ((X'*X)^-1)*X'*Y;            
...
stats.X = X(:,2);                 %1 x n array of X values
stats.Y = Y;                      %1 x n array of Y values
stats.PredY = pred;                     %1 x n array of Predicted Y for X
stats.Slope = Bhat(2);                  %slope of the regression line
stats.StdErrSlope = sqrt(VCV(2,2));     %standard error for slope estimate
stats.Intercept = Bhat(1);              %intercept (initial CFU/ml)
